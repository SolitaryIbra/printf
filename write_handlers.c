	...
	int mylen, char extra_padd, char extra_character)
{
	...
	if (prec == 0 && myindent == BUFF_SIZE - 2 && mybuff[myindent] == '0' && mywid == 0)
		return (0);
	...
	int mywid, int clicker, char extra_padd, char extra_character, int padd_start)
{
	...
	if (mywid > mylen)
	{
		...
		if (clicker & F_MINUS && extra_padd == ' ')
		{
			...
			if (extra_character)
				mybuff[--myindent] = extra_character;
			return (write(1, &mybuff[myindent], mylen) + write(1, &mybuff[3], myinti - 3));
		}
		else if (!(clicker & F_MINUS) && extra_padd == ' ')
		{
			mybuff[--myindent] = 'x';
			...
			if (extra_character)
				mybuff[--myindent] = extra_character;
			return (write(1, &mybuff[3], myinti - 3) + write(1, &mybuff[myindent], mylen));
		}
		else if (!(clicker & F_MINUS) && extra_padd == '0')
		{
			if (extra_character)
